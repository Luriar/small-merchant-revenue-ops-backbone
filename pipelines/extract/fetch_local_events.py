"""
Bronze extractor: Seoul local cultural events data.
"""
import shutil
...
def fetch(year: int, quarter: int, use_samples: bool = True, parent_run_id: Optional[str] = None) -> Path:
    ...
    try:
        if use_samples:
            sample_file = samples_path() / f"bronze_local_events_{year}Q{quarter}.csv"
            ...
            raise NotImplementedError("Live Seoul local events extraction not implemented in M3.")
