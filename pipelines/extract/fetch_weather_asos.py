"""
Bronze extractor: KMA ASOS daily weather observations.
"""
import shutil
...
def fetch(year: int, quarter: int, use_samples: bool = True, parent_run_id: Optional[str] = None) -> Path:
    ...
    run_id = create_run(
        ...
        input_ref="samples" if use_samples else "kma_asos_api",
    )
    mark_processing(run_id)

    try:
        if use_samples:
            sample_file = samples_path() / f"bronze_weather_asos_{year}Q{quarter}.csv"
            ...
            raise NotImplementedError("Live KMA ASOS API extraction not implemented in M3.")
