"""
Bronze extractor: Seoul estimated sales data.
In sample mode, copies from data/samples/revenue_ops_demo/.
In live mode, downloads from Seoul Open API or CSV.
"""
import shutil
...
SOURCE_NAME = "seoul_estimated_sales"
...
def fetch(year: int, quarter: int, use_samples: bool = True, parent_run_id: Optional[str] = None) -> Path:
    ...
    try:
        if use_samples:
            sample_file = samples_path() / f"bronze_seoul_sales_{year}Q{quarter}.csv"
            ...
            raise NotImplementedError(
                ...
                "Set USE_SAMPLE_DATA=true or provide downloaded CSV in bronze path."
            )
